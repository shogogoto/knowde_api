...
def test_create_cred(fn):
    repo.create(cred)
    assert repo.findById(userId) is not None
...
def test_update(fn):
    repo.create(cred)
    newpwd = "new_password"
    newCred = Credential(userId, PlainPassword(newpwd))
    repo.update(newCred)
    found = repo.findById(userId)
    assert found.userId == userId
...
#     new   = Credential(userId, new_passwd)
